...
//recebera os parametros para alterar a nota
$id = filter_input(INPUT_GET, "idNota");
$matricula= filter_input(INPUT_GET, "Matricula");		

//pesquisar a nota
$consulta = "SELECT * FROM nota WHERE matricula_alu_nota='$matricula' AND id_nota='$id'";
$resultado = mysqli_query($con,$consulta);

$conteudo = mysqli_fetch_assoc($resultado);

$nota = $conteudo['nota'];
$materia = $conteudo['materia_not'];
$epoca = $conteudo['epoca'];
$matprof = $conteudo['matricula_prof_nota'];
      ...
      <div class="row">
          ...
          <div class="box box-warning">
            <div class="box-header with-border"> 
              <h3 class="box-title">Alterar nota</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
            <form id="form" name="form" action="alterarnota.php" enctype="multipart/form-data" data-ls-module="form" method="post" class="form-horizontal">
              <div class="box-body">
               <input type="hidden" class="form-control" name="IdNota" value="<?php echo $id ?>">
               <input type="hidden" class="form-control" name="Matricula" value="<?php echo $matricula ?>">
               
                <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Nota<i class="text-blue">*</i></label>
                  
                  <div class="col-sm-10">
                    <span class="help-block" id="war-not" style="display:none;"><i class="fa fa-bell-o text-yellow"> Por favor preencha este campo</i></span>                  
                    <input type="text" class="form-control" id="nota" name="NotaAluno" placeholder="Nota" value="<?php echo $nota ?>">
                  </div>
                </div>
                <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Matéria<i class="text-blue">*</i></label>
                  ...
                  <div class="col-sm-10">
                    ...
                    <input type="text" class="form-control" id="materia" name="MateriaNota" placeholder="Matéria" value="<?php echo $materia ?>">
                  </div>
                </div>
                <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Época<i class="text-blue">*</i></label>
                  
                  <div class="col-sm-10">
                    <span class="help-block" id="war-epo" style="display:none;"><i class="fa fa-bell-o text-yellow"> Por favor selecione uma opção</i></span>                  
                    <select type="text" class="btn btn-default dropdown-toggle" data-toggle="dropdown" id="epoca" name="Epoca" >
                    	<option value="1°Trim" <?php echo $epoca=='1°Trim'?'selected':'';?> >1°Trim</option>
			<option value="2°Trim" <?php echo $epoca=='2°Trim'?'selected':'';?> >2°Trim</option>
			<option value="3°Trim" <?php echo $epoca=='3°Trim'?'selected':'';?> >3°Trim</option>
		    </select>
                  </div>
                </div>                
              <!-- /.box-body -->
              <div class="box-footer">
                <button type="button" onclick="voltar()" class="btn btn-default">Voltar</button>               
...
function alerta1(){
var nota = form.NotaAluno.value;
var materia = form.MateriaNota.value;
return	$.confirm({
title: 'Nota',
content: 'Deseja mesmo alterar essa nota?',
buttons: {
Sim: {
text: 'Sim',
btnClass: 'btn-blue',
...
if(nota == ""){
document.getElementById("war-not").style.display="inline";
form.NotaAluno.focus();
return true;								        
}else{
document.getElementById("war-not").style.display="none";
}

if(materia == ""){
document.getElementById("war-mat").style.display="inline";
form.MateriaNota.focus();
return true;								        
}else{
...
if(typeof window.history.replaceState == 'function') {
        window.history.replaceState({}, "Hide", "http://athena.troiatecnologia.com.br/new_sistem/pages/aluno/alterar_nota.php");
} 
</script>
</body>
</html>