...
include_once '../../dist/php/info.php';
	...
	$id = filter_input(INPUT_POST, "IdFreq");
	$matricula = filter_input(INPUT_POST, "Matricula");
	$freqV = filter_input(INPUT_POST, "FreqAluno");
	$dataV = filter_input(INPUT_POST, "Data");
	$freq = mb_strtoupper($freqV);
	...
	"ano da chamada: " . $ano . "</br>";
	"ano atual: " . $anoatual;
	
	//caso o ano seja igual, pode fazer o update com a data
	if($ano == $anoatual){
	
if ($con) {

$query = mysqli_query($con, "UPDATE frequencia SET chamada='$freq', data_freq='$data' WHERE matricula_alu_freq='$matricula' and id_freq='$id'");
...
$.confirm({
title: 'Sucesso!',
content: 'Frequência alterada com sucesso',
buttons: {
Ok: {
text: 'Ok',
btnClass: 'btn-blue',
keys: ['enter', 'shift'],
action: function(){
window.location = 'freq.php?Matricula=".$matricula."';
}
}
}
});
...
die("Erro: ". mysqli_error($con));
	...
	//se o ano nao for igual, nao alterar a data de modificação
	
if ($con) {

$query = mysqli_query($con, "UPDATE frequencia SET chamada='$freq' WHERE matricula_alu_freq='$matricula' and id_freq='$id'");
