...
//recebera os parametros para alterar a freq
$id = filter_input(INPUT_GET, "idFreq");
$matricula= filter_input(INPUT_GET, "Matricula");		

//pesquisar a freq
$consulta = "SELECT * FROM frequencia WHERE matricula_alu_freq='$matricula' AND id_freq='$id'";
$resultado = mysqli_query($con,$consulta);

$conteudo = mysqli_fetch_assoc($resultado);

$chamada = $conteudo['chamada'];
$data = $conteudo['data_freq'];
      ...
      <div class="row">
          ...
          <div class="box box-warning">
            <div class="box-header with-border"> 
              <h3 class="box-title">Alterar frequência</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
            <form id="form" name="form" action="alterarfreq.php" enctype="multipart/form-data" data-ls-module="form" method="post" class="form-horizontal">
              <div class="box-body">
               <input type="hidden" class="form-control" name="IdFreq" value="<?php echo $id ?>">
               <input type="hidden" class="form-control" name="Matricula" value="<?php echo $matricula ?>">
                <div class="form-group">
                   <label for="inputEmail3" class="col-sm-2 control-label">Data<i class="text-blue"></i></label>

                  <div class="col-sm-10">
                    <input type="hidden" class="form-control" id="dt" name="Data" value="<?php echo date('d/m/Y',strtotime($data)); ?>">
                    <input type="text" disabled class="form-control" id="dt" name="Data" value="<?php echo date('d/m/Y H:i:s',strtotime($data)); ?>">
                  </div>
                </div>               
                <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Chamada<i class="text-blue">*</i></label>
                  
                  <div class="col-sm-10">
                    <span class="help-block" id="war-cha" style="display:none;"><i class="fa fa-bell-o text-yellow"> Por favor preencha este campo</i></span>                  
                    <input type="text" class="form-control" id="chamada" name="FreqAluno" placeholder="P = Presente / F = Falta" value="<?php echo $chamada ?>">
                  </div>
                </div>
...
function alerta1(){
var chamada = form.FreqAluno.value;
return	$.confirm({
title: 'Frequência',
content: 'Deseja mesmo alterar essa frequência?',
buttons: {
Sim: {
text: 'Sim',
btnClass: 'btn-blue',
...
if(chamada == ""){
document.getElementById("war-cha").style.display="inline";
form.FreqAluno.focus();
return true;								        
}else{
document.getElementById("war-cha").style.display="none";
}
...
if(typeof window.history.replaceState == 'function') {
        window.history.replaceState({}, "Hide", "http://athena.troiatecnologia.com.br/new_sistem/pages/aluno/alterar_freq.php");
} 
</script>
</body>
</html>