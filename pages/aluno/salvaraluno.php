...
$matricula= filter_input(INPUT_POST, "Matricula");
	//se matricula for vazia, o php gerara uma matricula aleatoria de 7 digitos min 999 max 9.999
	if(empty($matricula)){
		$matricula = rand(999,9999);
	}
...
//query para buscar se matricula ja existe
$sql = "SELECT matricula_alu FROM aluno WHERE matricula_alu ='$matricula'";
...
echo "<script type='text/javascript'>
	$.confirm({
		title: 'Erro!',
		content: 'Esta matricula já está cadastrada',
		buttons: {
			Ok: {
			text: 'Ok',
			btnClass: 'btn-blue',
...
//verificar a conexÃ£o, se tudo estiver certo, vai executar a linha e enviar o novo registro, se nao vai informar qual o erro
if ($con) {

  if ($file_namefoto == '' && $file_size == 0 && $dt_nasc_resp2 == "0000-00-00"){
  
  
  		$query = mysqli_query($con, "insert into aluno values('','$matricula','$nome_aluno','$sexo','$file_namefoto', '$binariofoto', '$file_typefoto','$dateFormat', '$rg_aluno','$cpf_aluno','$turma_fk','$nome_resp','$cpf_resp','$rg_resp','$dateFormat2','$email_resp','$nome_resp2','$cpf_resp2','$rg_resp2','$dt_nasc_resp2','$email_resp2','$nome_respf','$cpf_respf','$rg_respf','$dateFormat4','$email_respf','$celular_respf','$logradouro_resp','$bairro_resp','$num_resp','$cep_resp','$cidade_resp','$tel_fixo_resp','$celular_resp','$celular_resp2','$resp_Seg_nome1','$resp_seg_tel1','$resp_seg_cpf1','$resp_Seg_nome2','$resp_seg_tel2','$resp_seg_cpf2','');");
...
iconv_set_encoding("internal_encoding", "UTF-8");		    
            ...
            $mensagem1  = "Responsavel {$nome_resp}<br />
            ...
            $mensagem2 = "Responsavel {$nome_resp2}<br />
            <strong>Usuario</strong>: {$email_resp2}<br />
            <strong>Senha</strong>: {$matricula}<br /> <br />
            Obrigado!<br /> <br /> 
            Esta é uma mensagem automática, por favor não responda!";
            
            $mensagem3 = "Responsavel {$nome_respf}<br />
		...
		$subject = utf8_decode($subject);
            ...
            $send_contact=mail($email_respf, $subject, $mensagem3, $headers); 
					...
					$.confirm({
					    title: 'Sucesso!',
					    content: 'Novo aluno adicionado com sucesso',
					    buttons: {
					        Ok: {
					            text: 'Ok',
					            btnClass: 'btn-blue',
					            keys: ['enter', 'shift'],
					            action: function(){
					                window.location = 'cadastrar_aluno.php';
					            }
					        }
					    }
					});
		...
		die("Erro: ".mysqli_error($con));
  		...
  		$query = mysqli_query($con, "insert into aluno values('','$matricula','$nome_aluno','$sexo','$file_namefoto', '$binariofoto', '$file_typefoto','$dateFormat', '$rg_aluno','$cpf_aluno','$turma_fk','$nome_resp','$cpf_resp','$rg_resp','$dateFormat2','$email_resp','$nome_resp2','$cpf_resp2','$rg_resp2','$dateFormat3','$email_resp2','$nome_respf','$cpf_respf','$rg_respf','$dateFormat4','$email_respf','$celular_respf','$logradouro_resp','$bairro_resp','$num_resp','$cep_resp','$cidade_resp','$tel_fixo_resp','$celular_resp','$celular_resp2','$resp_Seg_nome1','$resp_seg_tel1','$resp_seg_cpf1','$resp_Seg_nome2','$resp_seg_tel2','$resp_seg_cpf2','');");
