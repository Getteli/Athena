...
include_once '../../dist/php/info.php';
	...
	$id = filter_input(INPUT_POST, "IdNota");	
	$matricula = filter_input(INPUT_POST, "Matricula");
	$nota = filter_input(INPUT_POST, "NotaAluno");
	$materia_not = filter_input(INPUT_POST, "MateriaNota");
	$epoca = filter_input(INPUT_POST, "Epoca");	
	
	//verificação se a epoca ja existe nesta nota
	$queryv = mysqli_query($con,"SELECT epoca,id_nota FROM nota WHERE matricula_alu_nota='$matricula' AND materia_not='$materia_not' AND epoca='$epoca'");
	$conteudov = mysqli_fetch_assoc($queryv);
	$epocae = $conteudov['epoca'];
	$ide = $conteudov['id_nota'];
	...
	//o id é diferente, ai verifica se a epoca é igual
	
	if($epoca == $epocae){
	
	//agora a epoca tbm é igual mas o id diferente, entao nao muda
	
	echo "<script type='text/javascript'>
	$.confirm({
	title: 'Erro!',
	content: 'Já existe uma nota no trimestre escolhido',
	buttons: {
	Ok: {
	text: 'Ok',
	btnClass: 'btn-blue',
	...
	if ($con) {
		$query = mysqli_query($con, "UPDATE nota SET nota='$nota', materia_not='$materia_not', epoca='$epoca', data_not='$data' WHERE matricula_alu_nota='$matricula' AND id_nota='$id'");
		...
		if ($query) {
			...
			$.confirm({
			title: 'Sucesso!',
			content: 'Nota alterada com sucesso',
			buttons: {
			Ok: {
			text: 'Ok',
			btnClass: 'btn-blue',
			keys: ['enter', 'shift'],
			action: function(){
			window.location = 'nota.php?Matricula=".$matricula."';
			}
			}
			}
			});
