...
include_once '../../dist/php/info.php';
	...
	$idturma = filter_input(INPUT_POST, "IdTurma");
	$turmavem = filter_input(INPUT_POST, "Turma");
	
    	$turma2 = preg_replace(array("/(á|à|ã|â|ä)/","/(Á|À|Ã|Â|Ä)/","/(é|è|ê|ë)/","/(É|È|Ê|Ë)/","/(í|ì|î|ï)/","/(Í|Ì|Î|Ï)/","/(ó|ò|õ|ô|ö)/","/(Ó|Ò|Õ|Ô|Ö)/","/(ú|ù|û|ü)/","/(Ú|Ù|Û|Ü)/","/(ñ)/","/(Ñ)/"),explode(" ","a A e E i I o O u U n N"),$turmavem);
    
       $turma = eregi_replace('( )','-',$turma2);

    
	$turno = filter_input(INPUT_POST, "Turno");
	$num_alu = filter_input(INPUT_POST, "NumAlu");
	$matricula_prof = filter_input(INPUT_POST, "MatriculaProf");

	if ($con) {
		$query = mysqli_query($con, "update turma set num_turma='$turma', turno_turma='$turno', num_alu_turma='$num_alu',matricula_prof_turm='$matricula_prof' where id_turma='$idturma';");
		if ($query) {
					echo "<script type='text/javascript'>
					$.confirm({
					    title: 'Sucesso!',
					    content: 'turma alterada com sucesso',
					    buttons: {
					        Ok: {
					            text: 'Ok',
					            btnClass: 'btn-blue',
					            keys: ['enter', 'shift'],
					            action: function(){
					                window.location = 'buscar_turma.php';
					            }
					        }
					    }
					});
