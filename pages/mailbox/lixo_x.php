...
include_once '../../dist/php/conexao.php';
	...
	$queryv = mysqli_query($con,'SELECT remedio1, visu FROM agenda WHERE id_agen IN('.implode( ',', $arr ).')');
	...
	if($remedio == '1' && $visu == '0'){
		$queryu = mysqli_query($con, 'UPDATE agenda SET visu="2" WHERE id_agen IN('.implode( ',', $arr ).')');
	}else{
	//caso ele seja visu == 1, entao ele ja foi visualizado e nao precisa dessa alteracao.
	}
	
		$query = mysqli_query($con, 'UPDATE agenda SET lixo = "1" WHERE id_agen IN('.implode( ',', $arr ).')');
