...
if (is_numeric($parametro) || !empty($mes) || !empty($dia) || !empty($ano)) {
	...
	$get = "SELECT * from agenda WHERE lixo = '1' AND matricula_alu_agen='$parametro' OR matricula_prof_agen='$parametro' AND lixo = '1' OR MONTH(data_agen) = '$mes' AND lixo = '1' OR DAY(data_agen) = '$dia' AND lixo = '1' OR YEAR(data_agen) = '$ano' AND lixo = '1' ORDER BY id_agen DESC";
	...
	$get = "SELECT * from agenda WHERE lixo = '1' AND matricula_alu_agen='$matr' OR matricula_prof_agen='$matrP' AND lixo = '1' OR titulo_agen like '%$parametro%' AND lixo = '1' OR mensagem_age like '%$parametro%' AND lixo = '1' OR MONTH(data_agen) = '$mes' AND lixo = '1' OR DAY(data_agen) = '$dia' AND lixo = '1' OR YEAR(data_agen) = '$ano' AND lixo = '1' ORDER BY id_agen DESC";

}else{
	//assim que inicia, como não tem parametro, ele busca tudo
	$get = "SELECT * FROM agenda WHERE lixo = '1' ORDER BY id_agen DESC";
}
      ...
      <div class="row">
          ...
          <div class="box box-solid">
            ...
            <div class="box-body no-padding">
              <ul class="nav nav-pills nav-stacked">
                ...
                <li class="active"><a href="lixeira.php"><i class="fa fa-trash-o"></i> Lixeira</a></li>
              </ul>
            </div>
            <!-- /.box-body -->
          </div>
        ...
        <div class="col-md-9">
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 id="red-tit" class="box-title">Lixeira</h3>
		...
		$tp = $tr / $total_reg; // verifica o número total de páginas
              ...
              <div class="table-responsive mailbox-messages">
                <table class="table table-hover table-striped" id="tbex">
                  <tbody>
                  <form id="form" action="excluiragenda_x.php" method="post">  
                  <?php
			...
			$dt = $dados["data_agen"];
			$visu = $dados["visu"];
			$matalu = $dados["matricula_alu_agen"];
			$matprof= $dados["matricula_prof_agen"];
                  ?>
                  ...
                  <tr>
                    ...
                    <a style="display:none;" id="<?php echo 'ex'.$id ?>" href="<?php echo "excluiragenda.php?IdAgenda=" . $id ?>"><i class="fa fa-trash-o text-red"></i></a>
                    </td>
                    <td>
                    <a href="#" onclick="alerta4(<?php echo $id ?>)"><i class="fa fa-level-up"></i></a>
                    <a style="display:none;" id="<?php echo $id ?>" href="<?php echo "restaurar.php?IdAgenda=" . $id ?>"><i class="fa fa-level-up"></i></a>
                    </td>                    
                    <td class="mailbox-name"><a <?php echo "href='ler_agendad.php?id=".$id."&matalu=".$matalu."&matprof=".$matprof."'" ?>>
<?php
...
<?php 			
			if($dados['remedio1'] == "1"){
				...
				echo "Professor(a) " . $linha3['nome_prof'];
							
			}else if($dados['matricula_alu_agen']){
			
				$nomeAlu = $dados['matricula_alu_agen'];
				
				$sql4 = "SELECT nome_alu FROM aluno WHERE matricula_alu = '$nomeAlu'";
				$dados4 = mysqli_query($con,$sql4);
					
				$linha4 = mysqli_fetch_assoc($dados4);
	
				echo "Aluno(a) " . $linha4['nome_alu'];
			
			}else if($dados['remedio1'] == "turmaagendasistema"){
			
				echo "Turma " . $dados['turma_aluno'];
...
?>                    
		...
		<?php } ?>
            ...
            <div class="box-footer no-padding">
              <div class="mailbox-controls">
                ...
                <?php
                if($tr <= 0){
                echo "<button type='button' class='btn btn-default btn-sm disabled'><i class='fa fa-trash-o'></i>/<i class='fa fa-level-up'></i></button>";
                ...
                echo "<button type='button' id='excluirall' onclick='alerta1()' class='btn btn-default btn-sm'><i class='fa fa-trash-o'></i>/<i class='fa fa-level-up'></i></button>";
		...
		<a href="#" onclick="alerta5()" id="sealltudo2" style="display:none;">selecionar tudo e restaurar</a>		
		    ...
		    switch($tp){
		    	case ($tp < 0):
		    	        echo " <a href='lixeira.php?pagina=$anterior&parametro=$parametro&search=&busca_dia=$dia&busca_mes=$mes&busca_ano=$ano' class='btn btn-default btn-sm disabled' title='Anterior'><i class='fa fa-chevron-left'></i></a>";
                    		echo " <a href='lixeira.php?pagina=$proximo&parametro=$parametro&search=&busca_dia=$dia&busca_mes=$mes&busca_ano=$ano' class='btn btn-default btn-sm disabled' title='Próximo'><i class='fa fa-chevron-right'></i></a>";
                    	break;
		    	case ($tp <= 1):
		    	        echo " <a href='lixeira.php?pagina=$anterior&parametro=$parametro&search=&busca_dia=$dia&busca_mes=$mes&busca_ano=$ano' class='btn btn-default btn-sm disabled' title='Anterior'><i class='fa fa-chevron-left'></i></a>";
                    		echo " <a href='lixeira.php?pagina=$proximo&parametro=$parametro&search=&busca_dia=$dia&busca_mes=$mes&busca_ano=$ano' class='btn btn-default btn-sm disabled' title='Próximo'><i class='fa fa-chevron-right'></i></a>";
                    	...
                    	case ($pc == 1):
		    	        echo " <a href='lixeira.php?pagina=$anterior&parametro=$parametro&search=&busca_dia=$dia&busca_mes=$mes&busca_ano=$ano' class='btn btn-default btn-sm disabled' title='Anterior'><i class='fa fa-chevron-left'></i></a>";
                    		echo " <a href='lixeira.php?pagina=$proximo&parametro=$parametro&search=&busca_dia=$dia&busca_mes=$mes&busca_ano=$ano' class='btn btn-default btn-sm' title='Próximo'><i class='fa fa-chevron-right'></i></a>";
                    	break;                    	
                    	case ( $tp > $pc):
		    	        echo " <a href='lixeira.php?pagina=$anterior&parametro=$parametro&search=&busca_dia=$dia&busca_mes=$mes&busca_ano=$ano' class='btn btn-default btn-sm' title='Anterior'><i class='fa fa-chevron-left'></i></a>";
                    		echo " <a href='lixeira.php?pagina=$proximo&parametro=$parametro&search=&busca_dia=$dia&busca_mes=$mes&busca_ano=$ano' class='btn btn-default btn-sm' title='Próximo'><i class='fa fa-chevron-right'></i></a>";                    		                    	
			break;                      	       	
                    	case ($pc >= $tp):
		    	        echo " <a href='lixeira.php?pagina=$anterior&parametro=$parametro&search=&busca_dia=$dia&busca_mes=$mes&busca_ano=$ano' class='btn btn-default btn-sm' title='Anterior'><i class='fa fa-chevron-left'></i></a>";
...
$(document).ready(function () {
  $('body').on('click', '.seall', function () {
    ...
    /*checkbox desmarcado*/
    	...
    	document.getElementById("sealltudo2").style.display="none";    	
	...
	document.getElementById("sealltudo2").style.display="inline";	
...
//excluir ou rstaurar qtd X
function alerta1(){
if(jQuery("input:checkbox[class=checkbox]:checked").length) {
	return	$.confirm({
	title: 'Agenda',
	content: 'Deseja excluir esta(s) mensagem(ns) OU restaurar?',
	buttons: {
	Excluir: {
	text: 'Excluir',
	btnClass: 'btn-red',
	action: function (){
	//seta qual action vai ser executado
	document.getElementById('form').action = "excluiragenda_x.php"	
	//executa o action escolhido acima
	...
	action: function (){
	//seta qual action vai ser executado
	document.getElementById('form').action = "restaurar_x.php"	
	//executa o action escolhido acima
	document.getElementById('form').submit();	
	}
	},	
...
return	$.confirm({
title: 'Agenda',
content: 'Prosseguir com a Exclusão destas mensagens definitivamente?',
buttons: {
Sim: {
text: 'Sim',
btnClass: 'btn-blue',
action: function (){
window.location = "excluiragenda_all.php";
}
},
cancelar: function () {
...
return	$.confirm({
title: 'Agenda',
content: 'Prosseguir com a Exclusão desta mensagem definitivamente?',
buttons: {
Sim: {
text: 'Sim',
btnClass: 'btn-blue',
...
return	$.confirm({
title: 'Agenda',
content: 'Restaurar esta mensagem?',
buttons: {
Sim: {
text: 'Sim',
btnClass: 'btn-blue',
...
return	$.confirm({
title: 'Agenda',
content: 'Restaurar tudo ?',
buttons: {
Sim: {
text: 'Sim',
btnClass: 'btn-blue',
action: function (){
window.location = "restaurar_all.php";
}
},
cancelar: function () {
...
if(typeof window.history.replaceState == 'function') {
        window.history.replaceState({}, "Hide", "http://athena.troiatecnologia.com.br/new_sistem/pages/mailbox/lixeira.php");
}     	
</script>
</body>
</html>
